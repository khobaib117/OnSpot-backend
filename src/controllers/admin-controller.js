...
exports.getAllUsers = async (req, res) => {
    ...
    try {
        const users = await Users.find({}, { originalPassword: 0 })
            .sort({ createdAt: -1 })
        if (users.length === 0) {
            return res.status(200).json({
...
exports.deleteUser = async (req, res) => {
    ...
    try {
        const users = await Users.deleteOne({ _id: mongoose.Types.ObjectId(req.body.id) })
...
exports.updateUserAccess = async (req, res) => {
    ...
    try {
        const users = await Users.updateOne({ _id: req.body.id }, { access: req.body.access })
